...
/**
 * File直接继承于Object，实现了Serializable接口和Comparable接口。
 * 1.实现Serializable接口，意味着File对象支持序列化操作。
 * 2.实现Comparable接口，意味着File对象之间可以比较大小（File能直接被存储在有序集合如TreeSet、TreeMap中）
 */
public class File
    implements Serializable, Comparable<File>
{
     ...
     */
    /**
     * 检查文件是否具有无效路径。目前，检查中文件路径非常有限，并且仅含有Nul字符检查。
     * 返回true标示该路径绝对是无效/垃圾。但是返回false不能保证路径有效。
     * @return
     */
     ...
     */
    /**
     * 根据parent父路径和child子路径字符串创建一个新的File
     */
     ...
     */
    /**
     *通过将给定路径字符串转换为抽象路径名来创建一个新File实例。
     */
     ...
     */
    /**
     *根据parent父路径名字符串和child子路径名字字符串创建一个新File实例。
     */
     ...
     */
    /**
     * 返回由此抽象路径名父目录的路径名字符串；如果此路径名没有指定父目录，则返回null
     */
     ...
     */
    /**
     * 返回由此抽象路径名父目录的File实例；如果此路径名没有指定父目录，则返回null
     */
